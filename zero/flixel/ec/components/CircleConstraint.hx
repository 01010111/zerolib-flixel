...
class CircleConstraint extends Component
{
	 ...
	 */
	public function new(center:Vec2, angle:Float, radius:Float, match_angle:Bool = true)
	{
		this.match_angle = match_angle;
		this.center = center;
		this.p = Vec2.get().copy_from(center);
	 ...
	 * @param r	radius
	 */
	public inline function update_radius(r:Float) p.length = r;
